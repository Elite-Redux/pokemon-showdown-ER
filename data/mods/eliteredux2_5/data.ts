...
import {
	...
	AbilityListSchema,
	Ability,
} from "../../proto/AbilityList_pb.js";
import {SpeciesList, SpeciesListSchema, Species} from "../../proto/SpeciesList_pb.js";
...
function protocLocation() {
	...
	default:
		console.error(
			`No proto compiler available for platform ${platform()}`
		);
		throw new Error("No proto compiler available for platform");
	}
}
...
export function moveForId(move: MoveEnum): Move {
	moveMap =
		moveMap || Object.fromEntries(readMoves().moves.map((it) => [it.id, it]));
	return moveMap[move]!;
}
...
export function abilityForId(ability: AbilityEnum): Ability {
	abilityMap =
		abilityMap ||
		Object.fromEntries(readAbilities().ability.map((it) => [it.id, it]));
...
export function speciesForId(species: SpeciesEnum): Species {
	speciesMap =
		speciesMap ||
		Object.fromEntries(readSpecies().species.map((it) => [it.id, it]));
...
export function reverseEvosForId(species: SpeciesEnum): SpeciesEnum[] {
	if (!reverseEvoMap) {
		reverseEvoMap = readSpecies().species.reduce(
			(arr: { [key in SpeciesEnum]?: SpeciesEnum[] }, it: Species) => {
...
export function readItems(): ItemList {
	if (!itemList) {
		...
		for (const file of readdirSync(`./${ER_CONFIG_PATH}/items/`)) {
			...
			itemList.item.push(
				...readTextproto(ItemListSchema, `items/${file}`).item
			);
		}
	}
	return itemList;
...
export function itemForId(item: ItemEnum): Item {
	itemMap =
		itemMap || Object.fromEntries(readItems().item.map((it) => [it.id, it]));
	return itemMap[item]!;
}
