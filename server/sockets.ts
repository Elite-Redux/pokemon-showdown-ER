...
export class ServerStream extends Streams.ObjectReadWriteStream<string> {
	...
	constructor(config: {
		...
		try {
			...
			const StaticServer: typeof import("node-static").Server =
				require("node-static").Server;
			...
			if (e.message === "disablenodestatic") {
				...
				console.log(
					"Could not start node-static - try `npm install` if you want to use it"
				);
			}
		}
		...
		if (this.serverSsl) {
			...
			console.log(
				`Worker ${PM.workerid} now listening for SSL on port ${
					// @ts-ignore - if appssl exists, then `config.ssl` must also exist
					config.ssl.port
				}`
			);
	...
	onConnection(socket: import("sockjs").Connection) {
		console.debug("sockets.ts.onConnection");
		// For reasons that are not entirely clear, SockJS sometimes triggers
		// this event with a null `socket` argument.
		if (!socket) return;
		console.debug("onConnection with valid socket");
		...
		console.debug("onConnection isTrustedProxyIp", socket.remoteAddress);
