...
export class Battle {
	...
	boost(
		...
		effect: Effect | null = null, isSecondary = false, isSelf = false, noEvents = false
	) {
		if (this.event) {
		...
		if (!noEvents) boost = this.runEvent('ChangeBoost', target, source, effect, {...boost});
		boost = target.getCappedBoost(boost);
		if (!noEvents) boost = this.runEvent('TryBoost', target, source, effect, {...boost});
		let success = null;
		...
		for (boostName in boost) {
			...
			if (boostBy) {
				...
				if (!noEvents) this.runEvent('AfterEachBoost', target, source, effect, currentBoost);
		...
		if (!noEvents) this.runEvent('AfterBoost', target, source, effect, boost);
		if (success) {
